...
public class FourteenTwo {
	...
	class Z
	{
		private DataStorage _data_storage;

		
		public Z(WordFrequencyFramework wfapp, DataStorage data_storage)
		{
			this._data_storage = data_storage;
			Interface ref = () -> _print_Zs();
			wfapp.register_for_end_event(ref);	
		}
		
		public void _print_Zs()
		{
			String data_str = String.join(" ", this._data_storage._data);
			HashSet<String> h = new HashSet<String>();
			
			for(String word: data_str.split("\\s+"))
			{
				if(!this._data_storage._stop_word_filter.is_stop_word(word))
				{
					if(word.contains("z"))
					{
						h.add(word);
	...
	public static void main(String[] args) {
		...
		Z z = f.new Z(wfapp,data_storage);
		wfapp.run(args[0]);
	}
}
